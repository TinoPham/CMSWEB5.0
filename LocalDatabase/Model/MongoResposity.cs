...
namespace ConverterDB.Model
{
	internal class MongoResposity : DBResposityBase,IResposity
	{
		...
		{
			MongoCollection<T> moncollection = DBContext.GetCollection<T>(GetTableName(typeof(T)));
			return moncollection.AsQueryable<T>();
		}
		...
		{
			IEnumerable<dynamic> iquery = GetDBSet(EntityType);
			
			return filter == null ? iquery : iquery.Where(filter);
		}
		...
		{
			Attribute att = (EntityType.GetCustomAttributes(typeof(TableAttribute), false).FirstOrDefault() as Attribute);
		...
		private IMongoQuery QueryEQByKey(object Entity)
		{
			...
			IEnumerable<PropertyInfo> ie_pinfo = pinfos.Where(item => item.GetCustomAttributes(typeof(KeyAttribute), false) != null);
			IEnumerable<IMongoQuery> ie_iqueries = ie_pinfo.Select(item => BuildEQQuery(item.Name, item.GetValue(Entity, null)));
		...
		private IMongoQuery BuildQuery(object Entity)
		{
			...
			BsonValue bvalue = pinfos[0].GetValue(Entity, null) as BsonValue;
			IEnumerable<IMongoQuery> queries = pinfos.Select(item => BuildEQQuery(item.Name, item.GetValue(Entity, null)));
