(function () {
	define(['cms',
		...
		cms.register.controller('discountCtrl', discountCtrl);
		discountCtrl.$inject = ['$scope', '$rootScope', '$timeout', '$stateParams', 'cmsBase', 'AppDefine', 'rebarDataSvc', 'bamhelperSvc', 'chartSvc'];
		function discountCtrl($scope, $rootScope, $timeout, $stateParams, cmsBase, AppDefine, rebarDataSvc, bamhelperSvc, chartSvc) {
			var vm = this;
			$scope.GroupByFieldConst = { Site: 1, Employee: 2 };
			$scope.groupByField = true; //true is group by site name, else value is employee
			//$scope.discountData = [];
			$scope.ReportID = AppDefine.CannedReportType.Discount;
