(function () {
	define(['cms',
		...
		cms.register.controller('nosaleCtrl', nosaleCtrl);
		nosaleCtrl.$inject = ['$scope', '$rootScope', '$timeout', '$stateParams', 'cmsBase', 'AppDefine', 'rebarDataSvc', 'bamhelperSvc', 'chartSvc'];
		function nosaleCtrl($scope, $rootScope, $timeout, $stateParams, cmsBase, AppDefine, rebarDataSvc, bamhelperSvc, chartSvc) {
			var vm = this;
			$scope.GroupByFieldConst = { Site: 1, Employee: 2 };
			$scope.groupByField = true; //true is group by site name, else value is employee
			//$scope.nosaleData = [];
			$scope.ReportID = AppDefine.CannedReportType.NoSale;
