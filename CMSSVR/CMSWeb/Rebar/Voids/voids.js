(function () {
	define(['cms',
		...
		cms.register.controller('voidCtrl', voidCtrl);
		voidCtrl.$inject = ['$scope', '$rootScope', '$timeout', '$stateParams', 'cmsBase', 'AppDefine', 'rebarDataSvc', 'bamhelperSvc', 'chartSvc'];
		function voidCtrl($scope, $rootScope, $timeout, $stateParams, cmsBase, AppDefine, rebarDataSvc, bamhelperSvc, chartSvc) {
			var vm = this;
			$scope.GroupByFieldConst = { Site: 1, Employee: 2 };
			$scope.groupByField = true; //true is group by site name, else value is employee
			//$scope.voidData = [];
			$scope.ReportID = AppDefine.CannedReportType.Void;
