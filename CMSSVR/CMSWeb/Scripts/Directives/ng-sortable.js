(function () {
    ...
    define(['cms', '/Scripts/Sortable.min.js'], contextMenuDirective);

    function contextMenuDirective(cms, SortableMenu) {
        cms.register.directive('ngSortable', ngSortableFn);

        ngSortableFn.$inject = ['$parse'];

        function ngSortableFn($parse) {
            var removed,
				nextSibling;
            ...
            return {
                ...
                link: function (scope, $el, attrs) {
                    ...
                    menusortable = SortableMenu.create(el, Object.keys(options).reduce(function (opts, name) {
                        ...
                        angular.forEach([
                            ...
                            watchers.push(scope.$watch('ngSortable.' + name, function (value) {
                                if (value !== void 0) {
                                    ...
                                    if (!/^on[A-Z]/.test(name)) {
                                        menusortable.option(name, value);
                                    }
                                }
                            }));
                        });
