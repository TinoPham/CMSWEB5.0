...
(function ($, undefined) {
    ...
    var fc = $.fullCalendar = {
        version: "1.6.4-yearview"
    };
    var fcViews = fc.views = {};
    var startObj;
    var endObj;
    function startDate(val) {
        startDate = cloneDate(val);
    }
    function endDate(val) {
        endObj = cloneDate(val);
    }
	...
	-----------------------------------------------------------------------------*/
        ...
        function CalulateFYCalendar(date, type, inc) {
            var fyobject = {
                startDate: new Date(),
                endDate: new Date()
            ...
            switch (type) {
                case calendarType.Week_52_or_53:
                    if (inc >= 0) {
                        var temp = addDays(cloneDate(date), 370);
                        var day1 = new Date(temp.getFullYear(), fyDate.getMonth(), fyDate.getDate());
                        var day2 = new Date(temp.getFullYear(), temp.getMonth(), temp.getDate());
                        if (day1 >= day2) {
                            fyobject.endDate = cloneDate(temp);
                            fyobject.startDate = addDays(cloneDate(temp), -370)
                        }
                        else {
                            fyobject.endDate = addDays(cloneDate(date), 363);
                            fyobject.startDate = cloneDate(date);
                        }
                    }
                    else {

                        var day1 = new Date(date.getFullYear(), fyDate.getMonth(), fyDate.getDate());
                        var day2 = new Date(date.getFullYear(), date.getMonth(), date.getDate());
                        if (day1 <= day2) {
                            fyobject.endDate = cloneDate(date);
                            fyobject.startDate = addDays(cloneDate(date), -370);
                        }
                        else {
                            fyobject.endDate = cloneDate(date);
                            fyobject.startDate = addDays(cloneDate(date), -363);
                        }
                    }
                    break;
                ...
                case calendarType.FirstMonth:
                    if (inc >= 0) {
                        ...
                        fyobject.endDate = date;
                        fyobject.startDate = addDays(cloneDate(date), 1);
                        fyobject.startDate = addYears(fyobject.startDate, -1);
                ...
                case calendarType.Weeks_52:
                    ...
                    {
                        fyobject.endDate = date;
                        fyobject.startDate = addDays(date, -363);
                    }
                    break;
                case calendarType.Week_53:
                    if (inc >= 0) {
                        ...
                        fyobject.endDate = date;
                        fyobject.startDate = addDays(date, -370);
                    }
        ...
        function renderView(inc) {
            if (inc) {
                var numdiff = dayDiff(endObj, startObj);
                if (inc >= 0) {

                    startObj = addDays(cloneDate(endObj), 1);
                    var _temp = CalulateFYCalendar(cloneDate(startObj), fyType, inc);
                    endObj = cloneDate(_temp.endDate); //addDays(cloneDate(startObj), numdiff);

                }
                else {

                    endObj = addDays(cloneDate(startObj), -1);
                    var _temp = CalulateFYCalendar(cloneDate(endObj), fyType, inc);
                    startObj = cloneDate(_temp.startDate);
                }
            }
	...
	-----------------------------------------------------------------------------*/
        ...
        /* Date
	-----------------------------------------------------------------------------*/
        function editClick(Ex_function) {
            return Ex_function;
        }
        function edit() {
            return 1;
    ...
    function Header(calendar, options) {
        ...
        function render() {
            ...
            if (sections) {
                element = $("<div class='fc-header container' style='width:100%'/>")
                    .append(
                        $("<div class='row'/>")
        ...
        function renderSection(position) {
            var e = $("<div class='fc-header-" + position + " col-md-6 col-lg-6'/>");
                    ...
                    $.each(this.split(','), function (j, buttonName) {
                        if (buttonName == 'title') {
                            ...
                            if (calendar[buttonName]) {
                                if (buttonName == 'edit') {
                                    buttonClick = calendar['editClick'];
                                }
                                else
    ...
    */
    ...
    function YearView(element, calendar) {
        ...
        fyStyle = t.options.fyStyle;
        periodTitle = t.options.periodTitle;
        fyType = t.options.fyType;
        fyDate = t.options.fyDate;

        startObj = t.options.startDate;
        endObj = t.options.endDate;
        ...
        function render(date, delta) {
            ...
            t.title = startObj.getFullYear() == endObj.getFullYear() ? startObj.getFullYear() : startObj.getFullYear() + ' - ' + endObj.getFullYear();
    ...
    function BasicYearView(element, calendar, viewName) {
        ...
        // exports
        //  t.editClick = calendar.editClick;
	...
	------------------------------------------------------------*/
        ...
        function fyMonthName(fyArrRange) {

            var firstDay = new Date(fyArrRange[1].getFullYear(), fyArrRange[1].getMonth(), 1);
            var lastDay = new Date(fyArrRange[0].getFullYear(), fyArrRange[0].getMonth() + 1, 0);
            var n1 = dayDiff(lastDay, fyArrRange[0]);
            var n2 = dayDiff(fyArrRange[1], addDays(lastDay, 1));
            if (n1 > n2) {

                return htmlEscape(formatDate(fyArrRange[0], 'MMMM, yyyy'));
            }
            else
                return htmlEscape(formatDate(addDays(lastDay, 1), 'MMMM, yyyy'));
            //   return formatDate(fyArrRange[0], 'MMM') + ' - ' + formatDate(fyArrRange[1], 'MMM');
        ...
        function buildSkeleton(monthsPerRow, showNumbers) {
            ...
            var nbMonths = lastMonth - firstMonth;
            //startObj = cloneDate(t.startDate);
            //endObj = cloneDate(t.endDate);
            //-------------------------------------------------
            // var startObj = /*cloneDate(this.minDate);*/ new Date(2015, 0, 1); // duc
            //var endObj = /*cloneDate(this.maxDate);*/ new Date(2015, 11, 31); //duc
            fyCal = fiscalYearArrays(startObj, endObj, fyStyle);
            //--------------------------------------------------
            ...
            di = cloneDate(startObj);
            firstDay = cloneDate(startObj).getDay();
            s = '<div class="fc-year-main-table fc-border-separate container" style="width:100%;"><div class="row row-centered">';
            ...
            for (m = firstMonth; m < lastMonth; m++) {
                ...
                s += '<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4 col-centered"><div class="fc-year-monthly-td " style="' + display + '">';
                s += '<table class="fc-border-separate fc-year-month" style="width:100%;" cellspacing="0">' +
                    ...
                    '<i data-year="' + y + '" data-month="' + (m % 12) + '" href="#">' +
                    /*htmlEscape(monthName) + '</a>' +*/ periodTitle + ' ' + (m + 1) + '-' + htmlEscape(fyMonthName(fyCal[m])) + '</i>' +
                    '</div>' +
                    '</td></tr>' +
                    '<tr>';
                ...
                if (showWeekNumbers) {
                    s += "<th class='fc-week-number " + headerClass + "'>" + htmlEscape(weekNumberTitle) + "</th>";
                }
                ...
                var OddandEven = dayDiff(fyCal[m][1], fyCal[m][0]) + 1;
                var rowsNumber = (OddandEven % 7 != 0) ? parseInt((OddandEven / 7) + 1) : parseInt(OddandEven / 7);
                var rowsconst = (fyStyle === '445') && (m === 11) ? rowsNumber : 5;

                for (i = 0; i < rowsconst; i++) {
                    if (nwe) {
                        skipWeekend(di);
                    }
                    ...
                    rowsForMonth[m]++;
                    rowCnt++;
                    var wnum = dayDiff(di, startObj) / 7 + 1;
                    s += '<tr class="fc-week' + i + '">';
                    if (showWeekNumbers) {
                        s +=
                            "<td class='fc-week-number " + contentClass + "'>" +
                            "<div>" +
                                            ((i >= rowsNumber) ? "" : wnum) + //htmlEscape(formatDate(di, weekNumberFormat)) +
                            "</div>" +
                            "</td>";
                    }
                    for (j = 0; j < colCnt; j++) {
                        ...
                        s += '<td class="' + contentClass + ' fc-day fc-' + dayIDs[di.getDay()] + ' fc-day' + dayStr + '">' + // need fc- for setDayID
                          ...
                          //'<div class="fc-day-content" style="min-height:' +0+ 'px;">' +
                            //'<div style="position:relative;"></div>' +
                            //'</div>' +
                            '</div>' +
                            '</td>';
                        if (i < rowsNumber) addDays(di, 1);
                        if (nwe) {
                            skipWeekend(di);
                        }
                    }
                ...
                s += '</tbody></table>';
                s += '<div class="fc-year-monthly-footer"></div></div>';
                s += '</div>';
          ...
          //  s += '<td class="fc-year-month-border fc-last"></td>';
            ...
            dayBind(bodyCells);
            var _obj = document.getElementsByClassName("temp");
            if (_obj !== null) {
                $(".temp").remove();
            }
            daySegmentContainer = $('<div style="position:absolute;z-index:8;top:0;left:0;" class="temp"/>').appendTo(element);
        }

        function Gapof454or544(monthIndex, type) {
            var num;
            switch (type) {
                case '454':
                    {
                        num = (monthIndex == 1 || monthIndex == 4 || monthIndex == 7 || monthIndex == 10) ? 34 : 27;
                        break;
                    }
                case '544':
                    {
                        num = (monthIndex == 0 || monthIndex == 3 || monthIndex == 6 || monthIndex == 9) ? 34 : 27;
                        break;
                    }
                case '445':
                    {
                        num = (monthIndex == 2 || monthIndex == 5 || monthIndex == 8 || monthIndex == 11) ? 34 : 27;
                        break;
                    }
                default:
                    {
        ...
        function fiscalYearArrays(beginDate, endDate, type) {
            var numdays = dayDiff(endDate, beginDate);
            var _fyCal = [];
            var gapNum = 0;

            for (var monthIndex = 0; monthIndex < 12; monthIndex++) {
                var num = Gapof454or544(monthIndex, type);
                gapNum = gapNum + (monthIndex != 0 ? 1 : 0);

                var _temp_s = addDays(cloneDate(beginDate), gapNum);
                gapNum = gapNum + num;
                var _tem_e;
                if (monthIndex == 11) {
                    _tem_e = addDays(cloneDate(beginDate), gapNum + (numdays - 363))
                } else {
                    _tem_e = addDays(cloneDate(beginDate), gapNum);
                }
                _fyCal.push([cloneDate(_temp_s), cloneDate(_tem_e)]);
            }
            return _fyCal;
        }
        /**
        ...
        function updateCells() {
            ...
            //var startObj = new Date(2014,10,6); // duc
            //var endObj = new Date(2015,9,4) //duc
            //var fyCal = fiscalYearArrays(startObj,endObj,'');
                ...
                $(_sub).find('tbody > tr').each(function (iii, _tr) {
                    ...
                    $(_tr).find('td').not('.fc-week-number').each(function (ii, _cell) {
                        ...
                        if (d <= fyCal[i][1]) {
                            ...
                            var d1 = clearTime(new Date());
                            var d2 = clearTime(cloneDate(d));
                            
                            if (dayDiff(d1,d2) == 0 ) {
                                cell.find('div.fc-day-number').html('<button type="button" style="width:100%;" class="btn btn-default btn-sm btn-info active" ><span  class="ng-binding">' + ("0" + dayNumber).slice(-2) + '</span></button>');
                            } else {
                                cell.find('div.fc-day-number').html('<button type="button" style="width:100%;" class="btn btn-default btn-sm" ><span  class="ng-binding">' + ("0" + dayNumber).slice(-2) + '</span></button>');
                            }
                        }
        ...
        function rangeToSegmentsYear(startDate, endDate) {
            ...
            for (var row = 0; row < rowCnt; row++) {
                ...
                if (segmentCellOffsetFirst <= segmentCellOffsetLast) {
                    ...
                    if (!gridRow && segments.length && lenBefore >= 0 && lenBefore <= 14 && otherMonthDays[gridOffset + firstMonth][0] > 1) {
	...
	-------------------------------------------------------------------------------------------------*/
        ...
        function setVerticals(segments, doRowHeights) {
            ...
            for (var i = 0; i < rowContentElements.length; i++) {
                rowContentTops.push(
                    0//rowContentElements[i].position().top
                );
            }
