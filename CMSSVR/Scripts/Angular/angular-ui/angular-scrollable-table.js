		        ...
		        controller: ['$scope', '$element', '$attrs', function ($scope, $element, $attrs) {
		            ...
		            function _resetColumnsSize(tableWidth) {
		                ...
		                $element.find("table th").each(function (index, el) {
		                    ...
		                    var _width = el.css('width'); // el.data('width');
		                    if (/\d+%$/.test(_width)) {    //percentage
		                        _width = Math.ceil(tableWidth * _getScale(_width) / 100);
		                    }
		                    else if (/\d+px$/.test(_width)) {
		                        var matches = /(\d+)px$/.exec(_width);
		                        _width = matches[1];
		                    }
		                    else {
		    ...
		    return {
						...
						'<div ng-mouseenter="enter()" ng-mouseleave="leave()">' +
							...
							'<span class="orderWrapper">' +
								'<span class="order" ng-show="focused || isActive()" ' +
									...
									'<i ng-show="isAscending()" class="glyphicon icon-arrow-down-5"></i>' +
									'<i ng-show="!isAscending()" class="glyphicon icon-arrow-up-5"></i>' +
								'</span>' +
							'</span>' +
						'</div>' +
		        ...
		        link: function (scope, elm, attrs, tableController) {
		            ...
		            scope.isActive = function () {
		                var cur_sort = tableController.getSortExpr();
		                if (cur_sort === null || cur_sort == undefined)
		                    return false;
		                return cur_sort === expr;
