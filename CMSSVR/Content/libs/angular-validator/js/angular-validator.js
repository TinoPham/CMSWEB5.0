...
angular.module('angularValidator').directive('angularValidator', ['$timeout',
    function ($timeout) {
    	return {
    		restrict: 'A',
    		link: function (scope, element, attrs, fn) {
    			...
    			function checkElementValidity(element) {
    				...
    				if ("validator" in element.attributes) {
    					...
    					if (scopeForm[element.name]) scopeForm[element.name].$setValidity("angularValidator", isElementValid);
    			...
    			function updateValidationMessage(element) {
    				...
    				var defaultRequiredMessage = function () {
    					return "<i class=' icon-cancel'></i> Required";
    				};
    				var defaultInvalidMessage = function () {
    					return "<i class=' icon-cancel'></i> Invalid";
    				};
    				...
    				if (scopeElementModel.$dirty || element.form && scope[element.form.name] && scope[element.form.name].submitted) {

    					var inputgroup = element;
    					var stringParrentClass = element.parentElement.attributes['class'].textContent;
    					if (stringParrentClass.indexOf('input-group') >= 0) {
    					...
    					if (scopeElementModel.$error.required) {
    						...
    						if ("required-message" in element.attributes) {
    							angular.element(inputgroup).after(generateErrorMessage(element.attributes['required-message'].value));
    						}
    							// Display the default required message
    						else {
    							angular.element(inputgroup).after(generateErrorMessage(defaultRequiredMessage));
    						}
    					} else if (!scopeElementModel.$valid) {
    						// If there is a custom validation message add it
    						if ("invalid-message" in element.attributes) {
    							angular.element(inputgroup).after(generateErrorMessage(element.attributes['invalid-message'].value));
    						}
    							// Display the default error message
    						else {
    							angular.element(inputgroup).after(generateErrorMessage(defaultInvalidMessage));
    						}
    					}
    				}
    			}
    			...
    			function isValidationMessagePresent(element) {
    				...
    				var elementSiblings = angular.element(inputgroup).parent().children();
    			...
    			function updateValidationClass(element) {
    				...
    				if (formField.$dirty || element.form && scope[element.form.name] && scope[element.form.name].submitted) {
