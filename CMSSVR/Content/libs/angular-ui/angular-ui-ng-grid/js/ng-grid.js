...
angular.module('ngGrid.services').factory('$domUtilityService',['$utilityService', '$window', function($utils, $window) {
    ...
    domUtilityService.UpdateGridLayout = function($scope, grid) {
    	...
    	if (grid.$root) {
    		grid.elementDims.rootMaxW = grid.$root.width(); //Thang Pham, fix issue $root is null when handle resize event, Oct 13 2015.
...
var ngStyleProvider = function($scope, grid) {
    ...
    $scope.rowStyle = function (row) {
        var ret = { "top": row.offsetTop + "px", "height": $scope.rowHeight + "px", "padding-bottom": $scope.rowHeight + "px" };
        if (row.isAggRow) {
    ...
    $scope.viewportStyle = function () {
        $scope.slimscrollHeight = $scope.viewportDimHeight();
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('gridTemplate.html',
    ...
    "</div>\r" +
    "\n" +
    "<div class=\"ngViewport\" unselectable=\"on\" ng-viewport ng-class=\"{'ui-widget-content': jqueryUITheme}\" ng-style=\"viewportStyle()\" slimscroll=\"{height: 'slimscrollHeight'}\">\r" + //slimscroll=\"{height: 'slimscrollHeight'}\"
    "\n" +
