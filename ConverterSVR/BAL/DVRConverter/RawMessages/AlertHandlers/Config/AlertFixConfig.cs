...
namespace ConverterSVR.BAL.DVRConverter.RawMessages.AlertHandlers.Config
{
	public class AlertFixConfigs : Commons.SingletonClassBase<AlertFixConfigs>
	{
		...
		{
			string path = Path.Combine(AppSettings.AppSettings.Instance.AppData, Config_FileName);
			LoadConfig(path);
		}
		...
		public IEnumerable<AlertFixConfig> GetConfig(tAlertEvent alt)
		{
			if( alt == null)
				return null;
			IEnumerable<AlertFixConfig> cfg = configs.Where(it => it.ALertFixType.HasValue && it.ALertFixType.Value == alt.KAlertType);
			if( cfg.Any())
				return cfg;
			cfg = configs.Where(it => it.ALertType.HasValue && it.ALertType.Value == alt.KAlertType);
			return cfg;
		}
