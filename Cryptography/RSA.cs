...
namespace Cryptography
{
	public static class RSA
	{
		...
		public static void RsaKey(int keySize, out string publickey, out string privateKey)
		{
			publickey = null;
			privateKey = null;
			if(keySize <= 0)
				throw new NotSupportedException("keySize");
		...
		public static byte [] DefaultPrivateEncryption(byte [] data)
		{
			string private_key = Utils.EmbededResourceString( Utils.RSA_KEY_DEFAULT_PRIVATE );
			return PrivateEncryption(data, private_key);
		}

		public static byte [] PrivateEncryption(byte [] data, string privateKey)
		{
			...
			using (RSACryptoServiceProvider privateRsa = new RSACryptoServiceProvider())
			{
				...
				return PrivateEncryption( privateRsa, data);
			}
		}

		static byte [] PrivateEncryption(RSACryptoServiceProvider rsa, byte [] data)
		{
			if (data == null)
		...
		public static byte [] DefaultPublicDecryption(byte [] cipherData)
		{
			string publicKey = Utils.EmbededResourceString(Utils.RSA_KEY_DEFAULT_PUBLIC);
			return PublicDecryption(cipherData, publicKey);
		}
		public static byte [] PublicDecryption(byte [] cipherData, string publicKey)
		{
			...
			using (RSACryptoServiceProvider publicRsa = new RSACryptoServiceProvider())
			{
				...
				return PublicDecryption(publicRsa, cipherData);
			}
		}

		static byte [] PublicDecryption(RSACryptoServiceProvider rsa, byte [] cipherData)
		{
			if (cipherData == null)
