...
namespace PACDMSimulator.PACDMConverter.ConverterPOS
{
	internal class ConverterPOS : ConvertBase
	{
		const string SQL_Transact = "Select top 1 * FROM Transact Where TransactKey > '{0}'";
		const string SQL_Retail = "Select  * FROM Retail Where TransactKey = '{0}'";
		const string SQL_SubRetail = "Select  * FROM Retail Where RetailKey = '{0}'";
		const string SQL_Sensor = "SELECT * from [Sensor] order by OT_Start ASC";
		
		const string STR_Sensor = "Sensor";


		public ConverterPOS(ConvertDB LocalDB, Programset pset, HttpClientSingleton httpclient, CancellationToken CancelToken, MediaTypeFormatter dataformatter = null)
			: base(LocalDB, pset, httpclient, typeof(ConverterPOS).Namespace, CancelToken, dataformatter)
		{
		...
		private MessageResult ConvertPOSTransact(ConvertInfo covnertinfo, DataFileInfo Fileinfo)
		{
			...
			while (transact != null && !string.IsNullOrEmpty( transact.TransactKey))
			{
				...
				if (base.CancellingToken.IsCancellationRequested)
				{
					message_ret.ErrorID = ERROR_CODE.SERVICE_TERMINAL;
					break;
				}
