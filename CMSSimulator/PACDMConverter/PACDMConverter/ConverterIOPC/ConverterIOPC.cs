...
namespace PACDMSimulator.PACDMConverter.ConverterIOPC
{
	internal class ConverterIOPC : ConvertBase
	{
		...
		public ConverterIOPC(ConvertDB LocalDB, Programset pset, HttpClientSingleton httpclient, CancellationToken CancelToken, MediaTypeFormatter dataformatter = null)
			: base(LocalDB, pset, httpclient, typeof(ConverterIOPC).Namespace, CancelToken, dataformatter)
		{
		}
		...
		private ConvertMessage.MessageResult ConvertIOPCCount(ConvertInfo covnertinfo, DataFileInfo Fileinfo)
		{
			...
			for (int i = rowindex; i < tblCount.Rows.Count; i++)
			{
				...
				{
					message_ret = new ConvertMessage.MessageResult { ErrorID = ERROR_CODE.SERVICE_TERMINAL };
					break;
				}
		...
		private ConvertMessage.MessageResult ConvertIOPCDriveThrough(ConvertInfo covnertinfo, DataFileInfo Fileinfo)
		{
			...
			while(access != null && access.TD_ID > 0 && !CancellingToken.IsCancellationRequested)
			{
