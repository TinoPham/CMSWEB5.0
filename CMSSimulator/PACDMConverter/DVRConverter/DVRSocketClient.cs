	...
	internal class DVRSocketClient: IDisposable
	{
		...
		private void DVR_ReceiveCallback(IAsyncResult ar)
		{
			...
			try
			{
				...
				if( bytesRead == 0)
				{
					OnSocketEventArgs(new Events.SocketEvent { Sender = this, ErrorCode = ERROR_CODE.CMS_SOCKET_RECEIVED_ERROR, SockError =  SocketError.Shutdown, Message = ResourceManagers.Instance.GetResourceString(Commons.ERROR_CODE.CMS_SOCKET_SHUTDOWN, Utils.CultureInfo) });
					return;
				}
			...
			{
				OnSocketEventArgs(new Events.SocketEvent { Sender = this, ErrorCode = ERROR_CODE.CMS_SOCKET_RECEIVED_ERROR, SockError =  SocketError.SocketError, Message = ResourceManagers.Instance.GetResourceString(Commons.ERROR_CODE.CMS_SOCKET_ERROR, Utils.CultureInfo) });
			}
