	...
	class APIService
	{
		...
		public static int InstallService(string filepath, string servicename)
		{
			...
			if (schService != IntPtr.Zero)
				result = UnInstallService(servicename);
		...
		{
			IntPtr schSCManager = OpenSCManager(null, null, (uint)SCM_ACCESS.SC_MANAGER_CONNECT);
			if (schSCManager == IntPtr.Zero)
				return Marshal.GetLastWin32Error();
		...
		public static int StopService(string serviceName)
		{
			...
			if (schSCManager == IntPtr.Zero)
				return Marshal.GetLastWin32Error();
			IntPtr schService = OpenService(schSCManager, serviceName, (uint)SERVICE_ACCESS.SERVICE_STOP);
			if (schService == IntPtr.Zero)
			{
				CloseServiceHandle(schSCManager);
				return Marshal.GetLastWin32Error();
			...
			CloseServiceHandle(schService);
			CloseServiceHandle(schSCManager);
			while (GetServiceState(serviceName) == SERVICE_STATE.SERVICE_STOP_PENDING)
			{
		...
		public static int StartService(string serviceName)
		{
			...
			IntPtr schService = OpenService(schSCManager, serviceName, (uint)SERVICE_ACCESS.SERVICE_START);
			if (schService == IntPtr.Zero)
			{
				CloseServiceHandle(schSCManager);
				return Marshal.GetLastWin32Error();
			...
			CloseServiceHandle(schService);
			CloseServiceHandle(schSCManager);
			while (GetServiceState(serviceName) == SERVICE_STATE.SERVICE_START_PENDING)
			{
				System.Threading.Thread.Sleep(1000);
			}
			return result;
		...
		public static int ChangeServiceStart(string ServiceName, SERVICE_START service_start)
		{
			...
			IntPtr scManager = OpenSCManager(null, null, (int)SCM_ACCESS.GENERIC_READ);
			if (scManager.Equals(IntPtr.Zero))
				return Marshal.GetLastWin32Error();

			IntPtr service = OpenService(scManager, ServiceName, (int)SERVICE_ACCESS.SERVICE_CHANGE_CONFIG);
		...
		public static SERVICE_STATE GetServiceState(string serviceName)
		{
			...
			int ret = 0;
			IntPtr schService = OpenService(schSCManager, serviceName, (uint)SERVICE_ACCESS.SERVICE_QUERY_STATUS);
		...
		private static ServiceInfo QueryServiceInfo(string ServiceName)
		{
			...
			IntPtr scManager = OpenSCManager(null, null, (uint)SCM_ACCESS.SC_MANAGER_CONNECT);

			if (scManager.Equals(IntPtr.Zero))
				return new ServiceInfo();

			IntPtr service = OpenService(scManager, ServiceName, (int)SERVICE_ACCESS.SERVICE_QUERY_CONFIG);
			if (service.Equals(IntPtr.Zero))
				return new ServiceInfo();
